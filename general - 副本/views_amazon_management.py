...
from general.models import User, AmazonShop, OperationalAccount, Project
...
def get_amazon_shops_api(request):
    ...
    try:
        ...
        if operator_filter:
            operator_list = [int(o.strip()) for o in operator_filter.split(',') if o.strip().isdigit()]
            if operator_list:
                query = query.filter(ops_id__in=operator_list)
        
        if ops_group_filter:
            group_list = [g.strip() for g in ops_group_filter.split(',') if g.strip()]
            if group_list:
                user_ids = OperationalAccount.objects.filter(ops_group__in=group_list).values_list('user_id', flat=True)
                query = query.filter(ops_id__in=user_ids)
        ...
        if project_filter:
            project_list = [p.strip() for p in project_filter.split(',') if p.strip().isdigit()]
            if project_list:
                query = query.filter(project_id__in=[int(p) for p in project_list])
...
def create_amazon_shop_api(request):
    ...
    try:
        ...
        project = None
        if project_id and str(project_id).strip() not in ['', 'null', 'undefined']:
            try:
...
def bulk_update_project_api(request):
    ...
    批量更新店铺的项目归属
    请求体：{
        shop_ids: [1, 2, 3],  # 店铺ID列表
        project_id: 1  # 项目ID，null表示移除项目
    }
    """
    try:
        data = json.loads(request.body)
        ...
        if not shop_ids:
            return JsonResponse({'success': False, 'error': '未选择任何店铺'}, status=400)
        ...
        if 555 not in user_perm_codes and 551 not in user_perm_codes:
            return JsonResponse({
                'success': False,
                'error': '无操作权限，需要店铺管理员权限（551）或超管权限（555）'
            }, status=403)
        ...
        if project_id:
            try:
                ...
                return JsonResponse({'success': False, 'error': '项目不存在'}, status=400)

        # 批量更新
        with transaction.atomic():
            updated_count = AmazonShop.objects.filter(
                id__in=shop_ids,
                company=request.user.company
        ...
        return JsonResponse({
            'success': True,
            'message': f'成功更新 {updated_count} 个店铺的项目归属',
