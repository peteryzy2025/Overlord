"""
Django Management Command: run_theme_clustering
================================================
从散装 ASIN 标题提取指纹并进行拓扑聚类的全链路命令。

用法:
    # 增量模式 (只处理 denoising=False 的 ASIN)
    python manage.py run_theme_clustering
...
from theme.new_release_subject_cluster import AmazonThemeClusteringPipeline


class Command(BaseCommand):
    help = "从散装 ASIN 标题提取指纹并进行拓扑聚类"
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f"主题聚类 Pipeline 执行完成 ({mode} 模式)")
        )
