...
def product_list_page(request):
    ...
    recent_subjects_7d = (
        AmazonThemeNovelty.objects.filter(launch_date__gte=seven_days_ago)
    ...
    total_clusters = AmazonThemeClusterNovelty.objects.count()
    recent_clusters_7d = AmazonThemeClusterNovelty.objects.filter(
        created_at__gte=now - timedelta(days=7)
    ).count()
    recent_clusters_month = AmazonThemeClusterNovelty.objects.filter(
        created_at__gte=first_day_of_month
    ).count()
...
def _api_amazon_products_impl(request):
    try:
        ...
        subject_filter = data.get(
            "subject", ""
        ).strip()  # 前端productFilter也映射到subject
        ...
        if is_latest_deal:
            queryset = queryset.filter(
                product__is_latest_deal=(is_latest_deal == "true")
            )
        ...
        if report_status in ("reported", "unreported"):
            # 获取所有被举报且有效的ASIN
            reported_asins = (
                ThemeReport.objects.filter(is_active=True)
        ...
        score_trend_row = (
            ...
            .order_by("product_id", "crawl_date")
            .values("product_id", "crawl_date", "score")
        )
        score_trend_lookup = {}
        ...
        for history in current_page_items:
            ...
            if current_user and current_user.is_authenticated:
                ...
                is_reported_by_current_user = ThemeReport.objects.filter(
                    product=product, reporter=current_user, is_active=True
                ).exists()
            ...
            history_list.append(
                {
                    ...
                    # 产品首次抓取时间
                    "latest_crawled_at": history.crawled_at.strftime(
                        "%Y-%m-%d %H:%M:%S"
                    )
                    if history.crawled_at
                    else None,
                    # 当前记录抓取时间
                    ...
                    "theme_record": {
                        ...
                        "ai_infringement_words": theme_record.ai_infringement_words
                        if theme_record
                        else None,
        ...
        if not stats_data:
            ...
            latest_deal_distribution = {
                "latest_deal_count": AmazonThemeNovelty.objects.filter(
                    is_latest_deal=True
                ).count(),
                "latest_arrival_count": AmazonThemeNovelty.objects.filter(
                    is_latest_deal=False
                ).count(),
            }
...
def api_product_detail(request, asin):
    ...
    try:
        ...
        return JsonResponse(
            {"success": False, "message": f"获取产品详情失败: {str(e)}"}, status=500
        )
    ...
    try:
        ...
        for field in required_fields:
            if not data.get(field):
                return JsonResponse(
                    {"success": False, "message": f"缺少必填字段: {field}"}, status=400
                )
        ...
        if AmazonProduct.objects.filter(asin=asin).exists():
            return JsonResponse(
                {"success": False, "message": f"ASIN {asin} 已存在"}, status=400
            )

        # 验证产品类型是否在可选范围内
        product_type = data.get("product_type", "")
        valid_product_types = [
            choice[0] for choice in AmazonProduct.PRODUCT_TYPE_CHOICES
        ]
        if product_type and product_type not in valid_product_types:
            return JsonResponse(
                {"success": False, "message": f"无效的产品类型: {product_type}"},
                status=400,
            )
        ...
        return JsonResponse(
            {
                "success": True,
                "message": "产品创建成功",
                "data": {"asin": product.asin, "title": product.title},
            }
        )
        ...
        return JsonResponse(
            {"success": False, "message": f"创建产品失败: {str(e)}"}, status=500
        )
...
def api_update_product(request, asin):
    ...
    try:
        ...
        if "launch_date" in data:
            ...
            if launch_date:
                try:
                    product.launch_date = datetime.strptime(
                        launch_date, "%Y-%m-%d"
                    ).date()
        ...
        return JsonResponse(
            {"success": False, "message": f"更新产品失败: {str(e)}"}, status=500
        )
...
def api_delete_product(request, asin):
    ...
    try:
        ...
        return JsonResponse(
            {"success": False, "message": f"删除产品失败: {str(e)}"}, status=500
        )
...
def export_products_csv(request):
    try:
        ...
        response["Content-Disposition"] = (
            'attachment; filename="amazon_products_history.csv"'
        )
        ...
        for history in queryset:
            ...
            writer.writerow(
                [
                    ...
                    product.image_url,
                    product.launch_date.strftime("%Y-%m-%d")
                    if product.launch_date
                    else "",
                    ...
                    history.crawled_at.strftime("%Y-%m-%d %H:%M:%S")
                    if history.crawled_at
                    else "",
                ]
            )
        ...
        return JsonResponse(
            {"success": False, "message": f"导出CSV失败: {str(e)}"}, status=500
        )
...
def export_products_json(request):
    ...
    try:
        ...
        return JsonResponse(
            {"success": False, "message": f"导出JSON失败: {str(e)}"}, status=500
        )
...
def api_bulk_update_product_type(request):
    ...
    try:
        ...
        if not data.get("asins") or not data.get("product_type"):
            return JsonResponse(
                {"success": False, "message": "缺少必填字段: asins 或 product_type"},
                status=400,
            )
        ...
        return JsonResponse(
            {
                ...
                "data": {"updated_count": updated_count, "product_type": product_type},
            }
        )
...
def api_product_statistics(request):
    ...
    try:
        ...
        # 按产品类型统计
        type_stats = list(
            AmazonProduct.objects.values("product_type")
        ...
        # 图片统计
        with_images = (
            AmazonProduct.objects.exclude(image_url__isnull=True)
            .exclude(image_url="")
            .count()
        )
        ...
        return JsonResponse(
            {
                ...
                "data": {
                    ...
                    "image_statistics": {
                        ...
                        "with_images_percentage": round(
                            (with_images / total_products * 100), 2
                        )
                        if total_products > 0
                        else 0,
                    },
                    "launch_date_statistics": {
                        ...
                        "with_launch_date_percentage": round(
                            (with_launch_date / total_products * 100), 2
                        )
                        if total_products > 0
                        else 0,
                    },
        ...
        return JsonResponse(
            {"success": False, "message": f"获取统计信息失败: {str(e)}"}, status=500
        )
...
def api_product_suggestions(request):
    ...
    try:
        ...
        # 搜索ASIN
        asin_suggestions = AmazonProduct.objects.filter(asin__icontains=query).values(
            "asin", "title"
        )[:10]

        # 搜索标题
        title_suggestions = AmazonProduct.objects.filter(title__icontains=query).values(
            "asin", "title"
        )[:10]
        ...
        return JsonResponse(
            {"success": False, "message": f"获取搜索建议失败: {str(e)}"}, status=500
        )
...
def export_products_excel(request):
    try:
        ...
        if scope == "selected":
            ...
            if title_filter:
                queryset = queryset.filter(
                    ...
                    | Q(title_translation__icontains=title_filter)
                )
            subject_filter = request.GET.get("subject", "").strip()
            if subject_filter:
                queryset = queryset.filter(
                    Q(subject__icontains=subject_filter)
                    | Q(subject_translation__icontains=subject_filter)
                )
        ...
        return JsonResponse(
            {"success": False, "message": f"导出Excel失败: {str(e)}"}, status=500
        )
...
def api_report_product(request):
    ...
    try:
        ...
        if not asin:
            return JsonResponse(
                {"success": False, "message": "缺少ASIN参数"}, status=400
            )
        ...
        try:
            ...
            return JsonResponse(
                {"success": False, "message": f"未找到ASIN为 {asin} 的产品"}, status=404
            )
        ...
        if subject:
            # 找到所有相同主题的产品（忽略大小写）
            same_subject_products = AmazonThemeNovelty.objects.filter(
            ...
            for p in same_subject_products:
                ...
                existing_report = ThemeReport.objects.filter(
                    product=p, reporter=request.user
                ).first()
                ...
                if existing_report:
                    ...
                    ThemeReport.objects.create(
                        product=p, reporter=request.user, is_active=True
                    )
        else:
            # 如果没有主题，只举报当前产品
            related_asins.append(asin)
            ThemeReport.objects.update_or_create(
                product=product, reporter=request.user, defaults={"is_active": True}
            )

        # 计算状态与举报人映射
        for p in AmazonThemeNovelty.objects.filter(asin__in=related_asins):
            qs = ThemeReport.objects.filter(product=p, is_active=True).select_related(
                "reporter"
            )
        ...
        return JsonResponse(
            {"success": False, "message": f"举报失败: {str(e)}"}, status=500
        )
...
def api_unreport_product(request):
    ...
    try:
        ...
        return JsonResponse(
            {"success": False, "message": f"撤销举报失败: {str(e)}"}, status=500
        )
