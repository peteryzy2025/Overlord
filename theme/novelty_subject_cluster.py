...
class AmazonNoveltyClusteringPipeline:
    ...
    def _stage4_persistence(self, components: list[set[int]], reset: bool = False):
        ...
        existing_cluster_map: dict[str, AmazonThemeClusterNovelty] = {
            c.display_title: c
            for c in AmazonThemeClusterNovelty.objects.all()
        }
        logger.info(
        ...
        clusters_to_update_basic: list[AmazonThemeClusterNovelty] = []
        ...
        with transaction.atomic():
            ...
            if clusters_to_create:
                ...
                logger.info(
                    "[Stage 4] 创建 AmazonThemeClusterNovelty: %d 条",
        ...
        def _compute_stats(cluster: AmazonThemeClusterNovelty):
            ...
            # 全量模式: 不显式设 updated_at, 保持 auto_now 在 bulk_create 时设的值

            return cluster

        def _fallback_stats(cluster: AmazonThemeClusterNovelty):
            cluster.asin_count = 0
            cluster.new_asin_7d = 0
            cluster.burst_score = 0.0

        clusters_to_update_stats: list[AmazonThemeClusterNovelty] = []
        ...
        if clusters_to_update_stats:
            if reset:
                update_fields = ["asin_count", "new_asin_7d", "burst_score"]
            else:
                update_fields = [
                    "asin_count",
                    "new_asin_7d",
