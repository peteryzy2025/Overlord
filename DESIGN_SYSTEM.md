...
| 层级 | 类名 | 用途 |
|------|------|------|
| 卡片 | `.data-card` | 一体化卡片容器 |
| 卡片 | `.data-card-header` | 头部 Flex 区（左侧可放图标+标题+副标题，右侧放操作按钮） |
| 卡片 | `.data-card-title` | 标题文字（支持带 Font Awesome 图标前缀） |
| 卡片 | `.data-card-filter` | 筛选区（内部 Grid 布局） |
| 表格 | `.table-container` | 表格外层 |
| 表格 | `.table` | 表格本体 |
| 批量 | `.bulk-action-bar` | 批量操作条（默认隐藏） |
| 批量 | `.bulk-action-bar.visible` | 显示状态 |
| 批量 | `.selected-count` | 已选择数量文字 |
| 分页 | `.pagination-container` | 分页容器 |
...
所有表格样式统一在 `general/static/css/general_style_1.css` 中维护，页面禁止重写结构样式。

### 5.1 表头
- **背景**：纯 `var(--bg-secondary)`
- **禁止**：使用任何 `linear-gradient` 渐变背景
- **圆角**：在 `.data-card` 内部时，表头左右上圆角为 `0`（`.data-card .table th:first-child` / `:last-child`）
- **字体**：表头及单元格文字统一使用系统默认字体，**无特殊说明不准加特殊字体**（如 `monospace`、自定义艺术字体等）
- **尺寸**：表头 `padding: 1.25rem 1.5rem`，`font-size: 1rem`，必须**高于或等于**内容行，字体**比内容大一档**

### 5.2 单元格
- 在 `.data-card` 内部时，`padding: 1rem 1.5rem`
- 每一行都有 `border-bottom`
- **最后一行**必须有 `border-bottom`（`.table tbody tr:last-child td`）
