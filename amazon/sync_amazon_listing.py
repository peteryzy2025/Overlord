...
async def lingxing_api_info_getter(sid,offset):
    """
    获取领星店铺列表
    """
    # total: 数据总数
...
#======================构造表格所需数据======================
def get_lingxing_api_info(sid,offset):
    """
    获取领星API数据，返回JSON字符串
    如果API返回空或出错，返回None
    """
    try:
        info = asyncio.run(lingxing_api_info_getter(sid,offset))
        if not info:
...
if __name__ == "__main__":
    json_data = get_lingxing_api_info("521738",5)
