...
class ShopIngestStats:
    ...
    error_count: int = 0
    title_changed_count: int = 0  # 新增：title变化数量
    title_changed_list: list = None  # 新增：记录变化的ASIN列表
    
    def __post_init__(self):
        if self.title_changed_list is None:
            self.title_changed_list = []
...
def _upsert_one_base_listing(shop: LingXingAmazonShop, row: Dict, stats: ShopIngestStats) -> str:
    """
    更新或创建 AmazonListingV2 记录
    V2 唯一约束: asin + sid + fulfillment_channel_type
    """
    incoming_title = row["title"]
    incoming_active = bool(row["is_active"])
    fulfillment_channel = row.get("fulfillment_channel_type") or ""
    
    # 尝试查找现有记录（V2 使用 asin + sid + fulfillment_channel_type 唯一）
    listing = AmazonListingV2.objects.filter(
        sid=shop.sid,
        asin=row["asin"],
        fulfillment_channel_type=fulfillment_channel
    ).first()

    if listing is None:
        # 创建新记录
        AmazonListingV2.objects.create(
            sid=shop.sid,
            asin=row["asin"],
            fulfillment_channel_type=fulfillment_channel,
            lingxing_shop=shop,
            ...
            status=1 if incoming_active else 0,  # V2 用 status 字段
            ...
            # 销量数据（直接写入 V2 的新字段）
            sales_volume_1d=row.get("volume_1d"),
            sales_volume_7d=row.get("volume_7d"),
            sales_volume_14d=row.get("volume_14d"),
            sales_volume_30d=row.get("volume_30d"),
            # 销售额数据
            sales_amount_1d=row.get("amount_1d"),
            sales_amount_7d=row.get("amount_7d"),
            sales_amount_14d=row.get("amount_14d"),
            sales_amount_30d=row.get("amount_30d"),
            # 日均销量
            sales_avg_volume_7d=row.get("avg_volume_7d"),
            sales_avg_volume_14d=row.get("avg_volume_14d"),
            sales_avg_volume_30d=row.get("avg_volume_30d"),
        )
    ...
    if not title_changed and not active_changed:
        ...
        # V2 销量字段（带 sales_ 前缀）
        listing.sales_volume_1d = row.get("volume_1d")
        listing.sales_volume_7d = row.get("volume_7d")
        listing.sales_volume_14d = row.get("volume_14d")
        listing.sales_volume_30d = row.get("volume_30d")
        listing.sales_amount_1d = row.get("amount_1d")
        listing.sales_amount_7d = row.get("amount_7d")
        listing.sales_amount_14d = row.get("amount_14d")
        listing.sales_amount_30d = row.get("amount_30d")
        listing.sales_avg_volume_7d = row.get("avg_volume_7d")
        listing.sales_avg_volume_14d = row.get("avg_volume_14d")
        listing.sales_avg_volume_30d = row.get("avg_volume_30d")
        listing.save(update_fields=[
            "updated_at",
            "small_image_url", "seller_sku", "seller_rank", "seller_category_new",
            "sales_volume_1d", "sales_volume_7d", "sales_volume_14d", "sales_volume_30d",
            "sales_amount_1d", "sales_amount_7d", "sales_amount_14d", "sales_amount_30d",
            "sales_avg_volume_7d", "sales_avg_volume_14d", "sales_avg_volume_30d",
        ])
    ...
    listing.status = 1 if effective_active else 0  # 同步更新 status
    listing.updated_at = timezone.now()
    update_fields = ["title", "is_active", "status", "updated_at"]
    ...
    if title_changed:
        ...
        stats.title_changed_list.append({
            "asin": row["asin"],
            "old_title": old_title[:80] + "..." if len(old_title) > 80 else old_title,
            "new_title": incoming_title[:80] + "..." if len(incoming_title) > 80 else incoming_title,
        })
...
def ingest_shop_base_data(sid, page_num=1, max_pages: Optional[int] = None, shop_idx: int = 0, total_shops: int = 0) -> ShopIngestStats:
    ...
    # 计算实际要处理的页数
    actual_total_pages = total_page_num
    if max_pages is not None:
        actual_total_pages = min(total_page_num, max_pages)
    
    # 构建店铺进度前缀
    shop_prefix = f""
    if total_shops > 0:
        shop_prefix = f"[{shop_idx}/{total_shops}] "
    
    # 使用 tqdm 显示分页进度
    pbar_desc = f"{shop_prefix}📦 {shop.name or '未知店铺'}"
    for curr_page in tqdm(range(current_page_num, actual_total_pages + 1), 
                          desc=pbar_desc[:60],  # 限制长度避免错位
                          total=actual_total_pages - current_page_num + 1,
                          unit="页"):
        
        if max_pages is not None and curr_page > max_pages:
            break

        page_offset = page(curr_page)
        try:
...
def ingest_all_us_shops(max_shops: Optional[int] = None, max_pages_per_shop: Optional[int] = None) -> List[ShopIngestStats]:
    shops = list(LingXingAmazonShop.objects.select_related("amazon_shop__ops").filter(name__contains="US"))
    ...
    print(f"\n📊 共发现 {total_shops} 个店铺需要同步\n")

    all_stats = []
    total_title_changed = 0
    
    for idx, shop in enumerate(shops, 1):
        try:
            stat = ingest_shop_base_data(
                shop.sid, 
                page_num=1, 
            ...
            total_title_changed += stat.title_changed_count
            
            # 简化的完成日志（显示title变化数量）
            title_info = f" 📝标题变:{stat.title_changed_count}" if stat.title_changed_count > 0 else ""
            print(f"  ✅ {shop.name or '未知店铺'} - 创建:{stat.listing_created} 更新:{stat.listing_updated} 跳过:{stat.listing_skipped}{title_info}")
            
            # 如果有title变化，打印详情（只打印前3个）
            if stat.title_changed_count > 0:
                for item in stat.title_changed_list[:3]:
                    print(f"     📝 {item['asin']}")
                    print(f"        旧: {item['old_title']}")
                    print(f"        新: {item['new_title']}")
                if stat.title_changed_count > 3:
                    print(f"     ... 还有 {stat.title_changed_count - 3} 个title变化")
            ...
            print(f"  ❌ {shop.name or '未知店铺'} - 失败: {exc}")
    
    print(f"\n📈 同步完成: {len(all_stats)}/{total_shops} 个店铺成功")
    if total_title_changed > 0:
        print(f"📝 共 {total_title_changed} 个Listing的title发生变化（将触发风险重检）")
    return all_stats
